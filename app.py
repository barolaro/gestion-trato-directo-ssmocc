...
def get_secret(name: str, default: str = "") -> str:
    """Obtiene un Secret sin provocar una excepción si todavía no existe."""
    try:
...
def get_admin_client() -> Client | None:
    """
    Cliente exclusivo para escrituras.

    Para guardar desde Streamlit agregue SUPABASE_SERVICE_KEY en Secrets.
    Esta clave permanece únicamente en Python y nunca se envía al navegador.
    """
    url = get_secret("SUPABASE_URL")
...
def contract_payload_for_dashboard(
    ...
    """
    Convierte las columnas de Supabase al formato utilizado por el HTML:
    LIC["establecimiento||licitacion"].
    """
    establishment_names = {
...
# -----------------------------------------------------------------------------
# ADMINISTRACIÓN CONTRACTUAL NATIVA
# -----------------------------------------------------------------------------
...
def render_admin_panel(
    ...
    with st.expander("🔐 Administración contractual (Supabase)", expanded=False):
        if not get_secret("ADMIN_PASSWORD"):
            st.info(
                "Agrega ADMIN_PASSWORD en los Secrets de Streamlit para habilitar "
                "este módulo administrativo."
            )
            return
        ...
        if admin_client is None:
            st.warning(
                "La lectura ya funciona, pero para guardar debes agregar "
                "SUPABASE_SERVICE_KEY en los Secrets de Streamlit. "
                "No la publiques en GitHub ni en el HTML."
            )
            return
        ...
        with st.form("contract_form", clear_on_submit=False):
            ...
            with col_lead:
                renewal_lead = st.number_input(
                    ...
                    max_value=36,
                    value=int(
                        selected_contract.get("anticipacion_renovacion") or 6
                    ),
                    step=1,
                )
...
def inject_supabase_contracts(
    ...
    new_save = (
        ...
        "toast('Guarde los cambios desde Administración contractual en Streamlit');"
        "}"
        "}catch(e){}"
        "}"
    )
...
def main() -> None:
    ...
    contracts_payload = contract_payload_for_dashboard(
        contracts,
        establishments,
    )
